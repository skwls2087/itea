...
public class NaverController { 
	...
	public String getAuthorizationUrl(HttpSession session) { 
		...
		OAuth20Service oauthService = 
				new ServiceBuilder().apiKey(CLIENT_ID).apiSecret(CLIENT_SECRET) .callback(REDIRECT_URI).state(state) 
	...
	public String getUserProfile(OAuth2AccessToken oauthToken) throws IOException { 
		...
		oauthService.signRequest(oauthToken, request); Response response = request.send(); 
		return response.getBody(); 
		} 
	
	public void naverProfile() { 
		String token = "YOUR_ACCESS_TOKEN";
		// 네이버 로그인 접근 토큰; String header = "Bearer " + token; 
		// Bearer 다음에 공백 추가 
		try { 
